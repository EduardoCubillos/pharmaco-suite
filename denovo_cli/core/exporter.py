...
def export_candidates(
    ...
    if meeko_ok:
        ...
        success = _prepare_pdbqt_meeko(sdf_path, pdbqt_path, verbose)
        if not success:
            pdbqt_path = None
...
def _write_sdf(candidates: List[Dict], path: str, verbose: bool) -> int:
    ...
    for cand in candidates:
        ...
        # Propiedades como metadatos SDF
        mol3d.SetProp("_Name",        f"Candidata_{cand.get('rank', exported+1):03d}")
        mol3d.SetProp("Rank",         str(cand.get("rank", "")))
        mol3d.SetProp("Score",        f"{cand.get('score', 0):.4f}")
        mol3d.SetProp("Pharma_Score", f"{cand.get('pharma_score', 0):.4f}")
        mol3d.SetProp("QED",          f"{cand.get('qed', 0):.4f}")
        mol3d.SetProp("MW",           f"{cand.get('mw', 0):.2f}")
        mol3d.SetProp("LogP",         f"{cand.get('logp', 0):.2f}")
        mol3d.SetProp("PSA",          f"{cand.get('psa', 0):.1f}")
        mol3d.SetProp("SA_Score",     f"{cand.get('sa_score') or 0:.2f}")
        mol3d.SetProp("HBD",          str(cand.get("hbd", "")))
        mol3d.SetProp("HBA",          str(cand.get("hba", "")))
        mol3d.SetProp("Lip_Violations", str(cand.get("lip_violations", "")))
        mol3d.SetProp("SMILES",       cand["smiles"])
